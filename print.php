...
?></span></div>
	...
	if($TYPE=='ranking'):
	    if($stat==0):
  $sql="select a.jid,b.cannumber,b.fullname,a.canid,sum(score) as total from tblscore a inner join tblcandidate b on a.canid = b.canid where b.gender='$gender' group by canid order by total asc";
        endif;
         if($stat==1):
  $sql="select a.jid,b.cannumber,b.fullname,a.canid,sum(score) as total from tblcriteriascore a inner join tblcandidate b on a.canid = b.canid where b.gender='$gender' group by canid order by total asc";
        endif;
  endif;
  if($TYPE=='percentage'):
  $sql="select a.jid,b.cannumber,b.fullname,a.canid,sum(score)/'$judgenumber' as total from tblscore a inner join tblcandidate b on a.canid = b.canid where b.gender='$gender' group by canid order by total desc";
  endif;
  ...
  <table  class="table table-default" style="boder:1px solid black;width:50px">
        ...
        <?php
	         ...
	         <?php
	                ...
	                if($TYPE=='ranking' && $stat==1){
	               $sqlscore="select a.jid,b.cannumber,b.fullname,a.canid,sum(score) as total from tblcriteriascore a inner join tblcandidate b on a.canid = b.canid where b.gender='$gender' and jid='$jid' group by a.canid order by ABS(b.cannumber) asc";
	                }else{
	                $sqlscore="select a.jid,b.cannumber,b.fullname,a.canid,sum(score) as total from tblscore a inner join tblcandidate b on a.canid = b.canid where b.gender='$gender' and jid='$jid' group by a.canid order by ABS(b.cannumber) asc";
