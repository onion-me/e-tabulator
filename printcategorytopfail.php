...
while($rowtype = mysqli_fetch_array($restype)){
  ...
  $ssqqll="select * from tblcategorytop where category='$category'";
  ...
  $DB=$_GET['database'];
  if($TYPE=='percentage'):
  $sql="select a.jid,b.cannumber,b.fullname,a.category,a.canid,b.cannumber,sum(score)/'$num'/'$per'*100 as total from tblscoretop a inner join tblcandidate b on a.canid = b.canid where a.category = '$cathehe' and b.gender='$gender' group by canid order by total desc limit $eli";
  endif;
  if($TYPE=='ranking'):
      if($stat==0):
  $sql="select a.jid,b.cannumber,b.fullname,a.category,a.canid,b.cannumber,sum(score)/'$per'*100 as total from tblscoretop a inner join tblcandidate b on a.canid = b.canid where a.category = '$cathehe' and b.gender='$gender' group  by canid order by total asc limit $eli";
        endif;
         if($stat==1):
  $sql="select a.jid,b.cannumber,b.fullname,a.category,a.canid,b.cannumber,sum(score) as total from tblcriteriascoretop a inner join tblcandidate b on a.canid = b.canid where a.category = '$cathehe' and b.gender='$gender' group  by canid order by total asc limit $eli";
        endif;
  endif;
...
<?php  } ?>
  ...
  <table  class="table table-default" style="boder:1px solid black;width:50px">
            ...
            <?php 
            if($TYPE=='percentage'):
  $sqlcan="select a.jid,b.cannumber,b.fullname,a.category,a.canid,b.cannumber,sum(score)/'$num'/'$per'*100 as total from tblscoretop a inner join tblcandidate b on a.canid = b.canid where a.category = '$cathehe' and b.gender='$gender' group by canid order by ABS(b.cannumber) asc";
  endif;
  if($TYPE=='ranking'):
  $sqlcan="select a.jid,b.cannumber,b.fullname,a.category,a.canid,b.cannumber,sum(score)/'$per'*100 as total from tblscoretop a inner join tblcandidate b on a.canid = b.canid where a.category = '$cathehe' and b.gender='$gender' group  by canid order by ABS(b.cannumber) asc ";
  endif;
        ...
        <?php
	         ...
	         <?php
	                $sqlcrit="select criteria from tblcriteriatop where category='$category'";
	                $rescrit=mysqli_query($con,$sqlcrit);
	                $critcount=mysqli_num_rows($rescrit);
	                if($TYPE=='ranking'):
	               $sqlscore="select a.jid,b.cannumber,b.fullname,a.canid,sum(score) as total from tblcriteriascoretop a inner join tblcandidate b on a.canid = b.canid where b.gender='$gender' and jid='$jid' and category='$category' group by canid order by ABS(b.cannumber) asc";
	               endif;
	               if($TYPE=='percentage'):
	               $sqlscore="select a.jid,b.cannumber,b.fullname,a.canid,sum(score) as total from tblscoretop a inner join tblcandidate b on a.canid = b.canid where b.gender='$gender' and jid='$jid' and category='$category' group by canid order by ABS(b.cannumber) asc";
	               endif;
