...
?></span></div>
  ...
  $judgenumber = mysqli_num_rows($resshing);
	...
	if($TYPE=='ranking'):
	     if($stat==0):
  $sql="select a.jid,b.cannumber,b.fullname,a.canid,sum(score) as total from tblscore a inner join tblcandidate b on a.canid = b.canid where b.gender='male' group by canid order by total asc";
        endif;
         if($stat==1):
  $sql="select a.jid,b.cannumber,b.fullname,a.canid,sum(score) as total from tblcriteriascore a inner join tblcandidate b on a.canid = b.canid where b.gender='male' group by canid order by total asc";
        endif;
  endif;
  if($TYPE=='percentage'):
  $sql="select a.jid,b.cannumber,b.fullname,a.canid,sum(score)/'$num' as total from tblscore a inner join tblcandidate b on a.canid = b.canid where b.gender='male' group by canid order by total desc";
  endif;
...
?>
     ...
     <div class="table-responsive">
      <table class="table table-striped">
       <thead class="thead-inverse">
          ...
          $sqlcan = "select * from tblcandidate where gender='male'";
          $rescan = mysqli_query($con,$sqlcan);
          $candidatenumber = mysqli_num_rows($rescan);
          ?>
    ...
    <?php
      ...
      $DB=$_GET['db'];
      $sqljudge = "select * from tblusers where db = '$DB' and usertype='judge' and status='activated'";
      ...
      $con=mysqli_connect('localhost','root','',$DB);
      $sql="select * from tblscore where jid='$judgeshit'  and gender='male' group by jid";
       $sqlt="select * from tblscore where jid='$judgeshit' and gender='male'";
  ...
  $con=mysqli_connect('localhost','root','',$DB);
  $ssqqll="select * from tblcategory";
  ...
  $DB=$_GET['db'];
  if($TYPE=='percentage'):
  $sql="select a.jid,b.cannumber,b.fullname,a.category,a.canid,b.cannumber,sum(score)/'$num'/'$per'*100 as total from tblscore a inner join tblcandidate b on a.canid = b.canid where a.category = '$cathehe' and b.gender='male' group by canid order by total desc";
  endif;
  if($TYPE=='ranking'):
      if($stat==0):
  $sql="select a.jid,b.cannumber,b.fullname,a.category,a.canid,b.cannumber,sum(score)/'$per'*100 as total from tblscore a inner join tblcandidate b on a.canid = b.canid where a.category = '$cathehe' and b.gender='male' group  by canid order by total asc";
        endif;
        if($stat==1):
  $sql="select a.jid,b.cannumber,b.fullname,a.category,a.canid,b.cannumber,sum(score) as total from tblcriteriascore a inner join tblcandidate b on a.canid = b.canid where a.category = '$cathehe' and b.gender='male' group  by canid order by total asc";
        endif;
  endif;
  $res=mysqli_query($con,$sql);
  ?>
...
?>
        ...
        <?php 
	        ...
	        $resshing=mysqli_query($conyat,$sqlshing);
            	...
            	<?php 
            	    $sqll="select * from tbllock where jid='$jid' and category='$cathehe'";
