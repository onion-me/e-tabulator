<?php   session_start();
 ...
 and gender = '$gender'";
	...
	$sqlmini = "insert into tblcriteriascore values('null','$judge','$criteria','$scoremini','$gender','$category','$candidate')";	
	...
	$sqlcheck="select * from tblscore where category='$category' and jid='$judge' and canid='$candidate'";
	$rescheck=mysqli_query($con,$sqlcheck);
	$numrow=mysqli_num_rows($rescheck);
	
	$sql="insert into tblscore values(null,'$candidate','$judge','$category','$catpercent','$total','$gender',null)";
		$res=mysqli_query($con,$sql)or die(mysqli_error($con));
		if($res)
		{

			$candidate=trim($candidate);
			echo "<script>open('judgehome.php?category=$category#candidate$candidate','_self');</script>";
		...
		else
		{
			echo 'faled to spell it correctly';
              	...
              	echo "<script>alert('duplicates are not allowed'); open('judgehome.php?category=$category','_self');</script>";
          }
