...
<?php  } ?>
  ...
  <table  class="table table-default" style="boder:1px solid black;width:50px;height:auto">
        ...
        <?php
                         $DB=$_GET['database'];    
	                    $sql="select * from tblusers where db='$DB' and usertype='judge' and status='activated' order by uid";
	         ...
	         <?php
                  ...
                  $queryy="select * from tblcategory where category='$category'";
                  $resultt=mysqli_query($con,$queryy);
                  $singleRow = mysqli_fetch_row($resultt);
                  $sagot = $singleRow['2'];

	               $sqlscore="select a.jid,b.cannumber,b.fullname,a.canid,score/('$sagot'/100) as total from tblscore a inner join tblcandidate b on a.canid = b.canid where b.gender='$gender' and jid='$jid' and category='$category' group by canid order by ABS(cannumber) asc";
    ...
    <?php
            $conyat=mysqli_connect('localhost','root','','tesdacmi_users');
 $DB=$_GET['database'];
  $sqlshing="select * from tblusers where db='$DB'and usertype='judge' and status='activated' order by uid";
  $resshing=mysqli_query($conyat,$sqlshing);
